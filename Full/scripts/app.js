					...
					link : function (scope, ele, attrs) {
						...
						getRandomData = function () {
							...
							for (data.length > 0 && (data = data.slice(1)); data.length < totalPoints; )
								prev = data.length > 0 ? data[data.length - 1] : 50, y = prev + 10 * Math.random() - 5, y < 0 ? y = 0 : y > 100 && (y = 100), data.push(y);
	...
	angular.module("app.tables", []).controller("tableCtrl", ["$scope", "$filter", function ($scope, $filter) {
				...
				$scope.order = function (rowName) {
					if ($scope.row !== rowName)
						return $scope.row = rowName, $scope.filteredStores = $filter("orderBy")($scope.stores, rowName), $scope.onOrderChange()
				},
	...
	angular.module("app.task", []).factory("taskStorage", function () {
		...
		DEMO_TASKS = '[ {"title": "Finish homework", "completed": true}, {"title": "Make a call", "completed": true}, {"title": "Play games with friends", "completed": false}, {"title": "Shopping", "completed": false}, {"title": "One more dance", "completed": false}, {"title": "Try Google glass", "completed": false} ]', {
			...
			put : function (tasks) {
				...
				$scope.add = function () {
					var newTask;
					if (newTask = $scope.newTask.trim(), 0 !== newTask.length)
						return tasks.push({
							title : newTask,
							completed : !1
						}), logger.logSuccess('New task: "' + newTask + '" added'), taskStorage.put(tasks), $scope.newTask = "", $scope.remainingCount++
				},
				...
				$scope.completed = function (task) {
					if ($scope.remainingCount += task.completed ? -1 : 1, taskStorage.put(tasks), task.completed)
						return $scope.remainingCount > 0 ? 1 === $scope.remainingCount ? logger.log("Almost there! Only " + $scope.remainingCount + " task left") : logger.log("Good job! Only " + $scope.remainingCount + " tasks left") : logger.logSuccess("Congrats! All done :)")
				},
				...
				$scope.markAll = function (completed) {
					if (tasks.forEach(function (task) {
						...
						}), $scope.remainingCount = completed ? 0 : tasks.length, taskStorage.put(tasks), completed)
...
.call(this), function () {
	...
	angular.module("app.ui.ctrls", []).controller("NotifyCtrl", ["$scope", "logger", function ($scope, logger) {
				...
				$scope.random = function () {
					...
					type = void 0,
					type = value < 25 ? "success" : value < 50 ? "info" : value < 75 ? "warning" : "danger",
					...
					controller : ["$scope", "$element", "$attrs", "$location", function ($scope, $element, $attrs, $location) {
							...
							highlightActive = function (links, path) {
								...
								angular.forEach(links, function (link) {
									...
									$link,
									href;
									if ($link = angular.element(link), $li = $link.parent("li"), href = $link.attr("href"), $li.hasClass("active") && $li.removeClass("active"), 0 === path.indexOf(href))
				...
				return i18nDirective = {
					...
					updateText : function (ele, input, placeholder) {
						...
						"i18n-placeholder" === input ? (result = localize.getLocalizedString(placeholder),
							ele.attr("placeholder", result)) : input.length >= 1 ? (result = localize.getLocalizedString(input), ele.text(result)) : void 0
					},
					link : function (scope, ele, attrs) {
...
.call(this), function () {
	...
	angular.module("app.controllers", []).controller("AppCtrl", ["$scope", "$location", function ($scope, $location) {
				return $scope.isSpecificPage = function () {
					...
					_.contains(["/404", "/pages/500", "/pages/login", "/pages/signin", "/pages/signin1", "/pages/signin2", "/pages/signup", "/pages/signup1", "/pages/signup2"], path)
				},
				$scope.main = {
					brand : "iFlat",
					name : "Lisa Doe"
