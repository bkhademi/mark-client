(function() {
        ...
        return {
            ...
            link: function(scope, ele, attrs) {
                ...
                return data = [], totalPoints = 300, getRandomData = function() {
                    ...
                    y = prev + 10 * Math.random() - 5, 0 > y ? y = 0 : y > 100 && (y = 100), data.push(y);
    ...
    angular.module("app.task", []).factory("taskStorage", function() {
        ...
        return tasks = $scope.tasks = taskStorage.get(), $scope.newTask = "", $scope.remainingCount = filterFilter(tasks, {
            ...
            return $scope.remainingCount += task.completed ? -1 : 1, taskStorage.put(tasks), 
            task.completed ? $scope.remainingCount > 0 ? 1 === $scope.remainingCount ? logger.log("Almost there! Only " + $scope.remainingCount + " task left") : logger.log("Good job! Only " + $scope.remainingCount + " tasks left") : logger.logSuccess("Congrats! All done :)") : void 0;
    ...
    angular.module("app.ui.ctrls", []).controller("NotifyCtrl", [ "$scope", "logger", function($scope, logger) {
        ...
        return $scope.max = 200, $scope.random = function() {
            var type, value;
            value = Math.floor(100 * Math.random() + 10), type = void 0, type = 25 > value ? "success" : 50 > value ? "info" : 75 > value ? "warning" : "danger", 
            ...
            controller: [ "$scope", "$element", "$attrs", "$location", function($scope, $element, $attrs, $location) {
                ...
                return links = $element.find("a"), path = function() {
                    ...
                    return path = "#" + path, angular.forEach(links, function(link) {
                        var $li, $link, href;
                        return $link = angular.element(link), $li = $link.parent("li"), href = $link.attr("href"), 
    ...
    angular.module("app.controllers", []).controller("AppCtrl", [ "$scope", "$location", function($scope, $location) {
        return $scope.isSpecificPage = function() {
            var path;
            return path = $location.path(), _.contains([ "/404", "/pages/500", "/pages/login", "/pages/signin", "/pages/signin1", "/pages/signin2", "/pages/signup", "/pages/signup1", "/pages/signup2", "/login", "/signup", "/getting-started" ], path);
        }, $scope.main = {
            brand: "iFlat",
            name: "Lisa Doe"
        };
